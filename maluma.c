...
void entropia(int tipo[],int comienzo, int longitud,int *posicion,double *entropia)
{   int numelem=longitud+1-comienzo;
	...
	while (i < longitud)
	{
		if (tipo[i]!= tipo[i+1])
		{
            ...
            *entropia =(((double)((double)(i-comienzo)/(double)numelem)*entrizq) + (((double)((double)(longitud-comienzo)-(double)i)/(double)numelem)*entrdech));//voy por aqui
			if(*entropia<menor){
                posmenor=i;
                menor=*entropia;
            }
